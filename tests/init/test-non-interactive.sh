...
OUT=$("$ROOT/scripts/thiscode-init.sh" --non-interactive)
...
CLAUDE_DISCODE_VAULT=/tmp/fake-vault-xyz OUT2=$("$ROOT/scripts/thiscode-init.sh" --non-interactive --json 2>/dev/null || true)
# vault path 가 env 값 또는 빈값 (디렉토리 미존재) — 어떤 경우든 detect 실행됨
"$ROOT/scripts/thiscode-init.sh" --detect-only --json | jq '.vault' >/dev/null || { echo "FAIL: env override detect 실패"; exit 1; }

# CLAUDE_DISCODE_INIT_AUTO env 인식 (실제 install 안 하고 output 만 확인)
OUT3=$(CLAUDE_DISCODE_INIT_AUTO="phase-3-mcp,phase-5-mode-r-preflight" "$ROOT/scripts/thiscode-init.sh" --non-interactive)
