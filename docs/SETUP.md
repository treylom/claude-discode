# SETUP — claude-discode (개발자 / 일반 사용자)
...
ls ~/.claude/plugins/claude-discode/.claude-plugin   # 검증: 출력 있음
```

advanced (1-line installer): `curl -fsSL https://raw.githubusercontent.com/treylom/claude-discode/main/install.sh | bash`
...
```
/claude-discode:search "your query"   # 4-Tier fallback 자동
/claude-discode:km                     # KM wizard
/claude-discode:setup                  # 재설정 (Tier 추가/제거)
```
