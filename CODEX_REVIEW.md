---
title: claude-discode Codex Adversarial Review (1차)
date: 2026-05-12
...
1. **Codex 검증 호출 누락** — install.sh 에 `codex --version` / `codex exec` 체크 없음. `/claude-discode:start` checklist 가 claude · tmux · Discord 만 다룸.
...
5. **Claude Code 서버 기능 명시 누락** (재경님 추가 spec 10:41 KST) — `claude -p` 헤드리스 원샷 / MCP server / tmux session vs headless 분리 패턴.

## 회복 plan — 신규 파일 7건 + 정정 2건

### 신규 파일

1. **`commands/codex-check.md`** — `/claude-discode:codex-check` slash
   - `codex --version` 검증
   - `codex login` 상태 확인
   - 모델 picker 확인 (gpt-5.5 표시)
   - ephemeral test (`codex exec --no-stream "ping"`)

2. **`skills/claude-discode-shared-memory/SKILL.md`** + `references/protocol.md`
   - T1-T4 4-tier 정책
   - 사용자 vault 있/없음 분기 (없으면 plain markdown folder)
   - SHARED-INDEX.md 형식
   - Read-before-Edit 의무 + 동시쓰기 방지

3. **`skills/claude-discode-meetings/SKILL.md`** + `references/protocol.md`
...
4. **`skills/claude-discode-codex-bridge/SKILL.md`**
...
5. **`docs/03-shared-memory.md`** — 한국어 강의 톤 가이드 (4-tier + 사용 시나리오)

6. **`docs/06-claude-code-server.md`** ⭐ — 재경님 추가 spec
...
7. **`docs/08-debug-노하우.md`** — vault SHARED-INDEX.md + `agent-korea-daily/디버깅.md` 의 24+ 카테고리 강의 chapter 매핑.
...
8. **`install.sh`** Step 7 정정 + Step 5b 추가 (Codex CLI 검증)
   ...
   - claude-discode plugin install 안내 (`/plugin marketplace add treylom/claude-discode`)
   - 신규 슬래시 4종 안내 ( `/claude-discode:start` / `:add-bot` / `:open-meeting` / `:self-update` / `:codex-check`)
...
| # | 자산 | 경로 |
|---|---|---|
| 1 | SHARED-INDEX.md (4-tier + 디버깅 24+) | `/Users/tofu_mac/obsidian-ai-vault/AI_Second_Brain/.claude-memory/shared/SHARED-INDEX.md` |
| 2 | tofu-at-codex slash | `/Users/tofu_mac/obsidian-ai-vault/.claude/commands/tofu-at-codex.md` |
| 3 | codex-exec-bridge skill (6-file) | `/Users/tofu_mac/obsidian-ai-vault/.claude/skills/codex-exec-bridge/` |
| 4 | pumasi-config-spec | `/Users/tofu_mac/obsidian-ai-vault/.claude/skills/pumasi-config-spec.md` |
| 5 | soul.md anatomy (6 봇) | `/Users/tofu_mac/.claude/channels/discord-<bot>/soul.md` |
| 6 | 디버깅 DB #1~#85 | `/Users/tofu_mac/obsidian-ai-vault/agent-korea-daily/디버깅.md` |
| 7 | headless claude pattern | `/Users/tofu_mac/obsidian-ai-vault/AI_Second_Brain/.claude-memory/shared/reference_headless_claude_oneshot_pattern.md` |
| 8 | meetings 4-file 표준 | `/Users/tofu_mac/obsidian-ai-vault/AI_Second_Brain/.claude-meetings/` |
